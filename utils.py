...
def apply_non_linearity(tensor, non_linearity):
...
def get_random_mask(features, r):
...
def top_k(raw_graph, K, device='cuda'):
...
def knn_fast(X, k, b, device = 'cuda'):
...
def knn_fast_anchor(X, k, b, m=1000, device='cuda'):
    """ m: number of anchors to sample"""
    X = F.normalize(X, dim=1, p=2)
    index = 0
    values = torch.zeros(X.shape[0] * (k + 1)).to(device)
    rows = torch.zeros(X.shape[0] * (k + 1)).to(device)
    ...
    norm_row = torch.zeros(X.shape[0]).to(device)
    norm_col = torch.zeros(X.shape[0]).to(device)
    anchor_index = getAnchorIndex(X.shape[0],m)
    X_anchor = X[anchor_index]
    while index < X.shape[0]:
        if (index + b) > (X.shape[0]):
            end = X.shape[0]
        else:
        ...
        sub_tensor = X[index:index + b]
        similarities = torch.mm(sub_tensor, X_anchor.t()) 
        vals, inds = similarities.topk(k=k + 1, dim=-1)
        inds = torch.tensor(anchor_index[inds.cpu()]).to(device)
        values[index * (k + 1):(end) * (k + 1)] = vals.view(-1)
        cols[index * (k + 1):(end) * (k + 1)] = inds.view(-1)
        rows[index * (k + 1):(end) * (k + 1)] = torch.arange(index, end).view(-1, 1).repeat(1, k + 1).view(-1)
        norm_row[index: end] = torch.sum(vals, dim=1)
